...
def main():
  ...
  parser.add_argument('--config', type=str, help='config path ex. config/sample/check_correlation/point_mass.yml')
  ...
  for episode in range(episodes):
    ...
    I_tn1 = torch.tensor(video.copy().transpose(2, 0, 1)[np.newaxis, :, :, :]/255).to(torch.float32).to(cfg["device"])

    for _ in range(100):
      action = np.array([np.random.normal(direction[0], 0.1), np.random.normal(direction[1]/2., 0.1)])
      time_step = env.step(action)
      u = torch.tensor(action[np.newaxis, :]).to(torch.float32).to(cfg["device"])

      video = env.physics.render(64, 64, camera_id=0)
      I_t = torch.tensor(video.copy().transpose(2, 0, 1)[np.newaxis, :, :, :]/255).to(torch.float32).to(cfg["device"])

      rec_I_t, rec_I_tp1, x_q_t, x_p_tp1 = model.estimate(I_t, I_tn1, u)

      observation_position.append(time_step.observation["position"])
      latent_position.append(x_q_t.to("cpu").detach().numpy()[0])
      # plt.cla()
      # art_1 = ax1.imshow(I_t.to("cpu").detach().to(torch.float32).numpy()[0].transpose(1, 2, 0))
      # art_2 = ax2.imshow(rec_I_t.to("cpu").detach().to(torch.float32).numpy()[0].transpose(1, 2, 0))
      # art_3 = ax3.imshow(rec_I_tp1.to("cpu").detach().to(torch.float32).numpy()[0].transpose(1, 2, 0))
      # art_4 = ax4.scatter(x_q_t.to("cpu").detach().numpy()[:, 0], x_q_t.to("cpu").detach().numpy()[:, 1], s=1.)
      # plt.pause(0.01)
  ...
  value = np.corrcoef(all_observation_position[:, 0], all_latent_position[:, 0])
  print(value)
  value = np.corrcoef(all_observation_position[:, 0], all_latent_position[:, 1])
  print(value)
  value = np.corrcoef(all_observation_position[:, 1], all_latent_position[:, 0])
  print(value)
  value = np.corrcoef(all_observation_position[:, 1], all_latent_position[:, 1])
  print(value)
