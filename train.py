...
def data_loop(epoch, loader, model, device, beta, train_mode=False):
    ...
    for batch_idx, (I, u) in enumerate(tqdm(loader)):
        ...
        if train_mode:
            mean_loss += model.train({"I": I.to(device).permute(1, 0, 2, 3, 4), "u": u.to(device).permute(1, 0, 2), "beta": beta}) * batch_size
        else:
            mean_loss += model.test({"I": I.to(device).permute(1, 0, 2, 3, 4), "u": u.to(device).permute(1, 0, 2), "beta": beta}) * batch_size
...
def main():
    ...
    with tqdm(range(1, cfg["epoch_size"]+1)) as pbar:
        ...
        for epoch in pbar:
            ...
            # Training phase #
            #================#
            train_loss = data_loop(epoch, train_loader, model, cfg["device"], beta, train_mode=True)
            ...
            if epoch % cfg["check_epoch"] == 1:
                ...
                for step in range(0, cfg["dataset"]["train"]["sequence_size"]-1):
                    ...
                    I_t, I_tp1, x_q_t, x_p_tp1 = model.estimate(
                            I.to(cfg["device"]).permute(1, 0, 2, 3, 4)[step+1],
                            I.to(cfg["device"]).permute(1, 0, 2, 3, 4)[step],
                            u.to(cfg["device"]).permute(1, 0, 2)[step+1])
