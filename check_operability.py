...
def main():
    ...
    for episode in tqdm(range(0, 90)):
        ...
        for _ in range(400):
            ...
            x_q_t = model.encoder.sample_mean({
                "I_t": observation.permute(2, 0, 1)[np.newaxis, :, :, :].to(cfg["device"]), "y_t": label.to(cfg["device"])})
            reconstructed_image = model.decoder.sample_mean({"x_t": x_q_t, "y_t": label.to(cfg["device"])})
